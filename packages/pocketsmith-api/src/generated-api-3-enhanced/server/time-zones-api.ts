...
export const TimeZonesApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async timeZonesGet(options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<Array<TimeZone>>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.timeZonesGet(options);
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['TimeZonesApi.timeZonesGet']?.[localVarOperationServerIndex]?.url;
...
export class TimeZonesApi extends BaseAPI {
    ...
    public timeZonesGet(options?: RawAxiosRequestConfig) {
        return TimeZonesApiFp(this.configuration).timeZonesGet(options).then((request) => request(this.axios, this.basePath));
    }
}
